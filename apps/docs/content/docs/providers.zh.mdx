...
|---|---|---|---|---|---|
| **Antigravity** | Google | ✅（`--conversation <id>`）| ❌ | `.agent_context/skills/` （fallback）| 由 Antigravity CLI 自己管理 |
...
Google 出品。CLI 二进制名为 `agy`，搭配 Google Antigravity 服务，默认走 Gemini 系列模型。**会话恢复真用**——通过 `--conversation <id>`；因为 stdout 是纯文本而非结构化事件流，守护进程从 CLI 的日志文件里抓取 conversation UUID。CLI 没有 `--model` flag——模型选择保存在 Antigravity 自己的设置里。
...
fallback 路径对应的工具是否真的读取这个目录，取决于工具本身的文档——没保证。如果你的 skill 对 Antigravity / Gemini / Hermes / OpenClaw 没起效，先查这个问题。
